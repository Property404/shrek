...
TEST_CASE(parsing, {
    ...
    REQUIRE_EQUALS(root.getProperty("#size-cells").getValueAsScalar<uint32_t>(), 1);
    REQUIRE_EQUALS(root.getProperty("#address-cells").getValueAsScalar<uint32_t>(), 1);

    const auto& memory_reg = root.getNode("memory").getProperty("reg").getValueAsSlice<uint32_t>();
    REQUIRE_EQUALS(memory_reg[0], 0xc0000000);
    REQUIRE_EQUALS(memory_reg[1], 0x04000000);
